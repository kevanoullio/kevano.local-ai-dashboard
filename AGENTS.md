...
(category decides order; tiers rarely compete for the same field):

- **Static / model fields** (size, layers, context, quant, dtype):
  **API → model file (GGUF) → models.ini preset.**
- **Runtime / location fields** (weight bytes per device, KV cache, DRAM/VRAM,
  CPU/GPU split): **exact service-args split → measured per-PID probe →
  GGUF-derived estimate → preset default.**

Marker convention:

- **Exact** — no prefix (verbatim from API/model file, or derived without
  estimation).
- **`~` estimate / measured** — prefix `~` (GGUF-derived KV upper bound,
  measured footprint, derived CPU/GPU percentage, any fallback).
- **`—`** — no source answered (unknown). Never guess.

### Golden rules (do not violate)

1. **Never use total-GPU VRAM as a per-model proxy.** GPU memory is per-PID
   only: `nvidia-smi --query-compute-apps=pid,used_memory` (NVIDIA) or
...
4. **Device placement comes from the resolved `--n-gpu-layers` (or the preset)**,
   never by dividing measured VRAM. No explicit offload → layers/percentages are
   `—`.
5. **Preset values arrive as the resolved CLI the service was launched with** —
   `GET /v1/models[].status.args`, parsed in `_parseLlamaArgs`. NOT by scanning
   `models.ini` and NOT from journald (journald is display-only, for "View debug
   output"). A direct `models.ini` scan is the **service-stopped** fallback only.

## Field → source (condensed, llama.cpp loaded models)

| Field | Primary source | Tier |
|---|---|---|
| name/id, loaded status | `/v1/models` `m.id`, `status.value` | 1 |
| model size | `meta.size`; fallback `.gguf` `stat` size (`ggufScript`) | 1 → 2 |
| effective context | `meta.n_ctx`; per-slot `/slots` → `n_ctx` | 1 |
| layers (total/main/MTP) | GGUF `block_count`, `nextn_predict_layers`, `full_attention_interval` | 2 |
| GPU/CPU split + % | resolved `--n-gpu-layers` in `status.args` (+ totals) | 1 |
| weight bytes per device | layer-ratio split of size; `~` on measured fallback | 1→2 |
| KV cache size / dtype / placement | GGUF estimate (`~`) / `status.args` `--cache-type-k/v` / `--no-kv-offload` | 2 / 1 |
| quant | `meta.ftype` (planned); GGUF `general.file_type` (planned) | 1 / 2 |
| service DRAM / VRAM | cgroup `anon+shmem` / per-PID GPU memory | 3 |
| service CPU/GPU split | DRAM ÷ (DRAM + VRAM) | 3 |
...
|---|---|---|
| 1 | `/v1/models`, `/slots` | `_finishJsonModels`, `_parseLlamaArgs`, `_finishSlots` |
| 2 | `ggufScript` bounded 16 KiB header read | `_queueGguf` → `ggufProcess` → `_finishGguf` → `_applyGguf` / `_applyGgufDraft` / `_applyGgufToRunning` |
| 3 | cgroup + GPU drivers | `serviceMemoryScript` / `serviceVramScript` → `_finishServiceMemory` / `_finishServiceVram` → `_deriveServiceTotal` |
| Display | `~` / `—` rendering, per-device totals | `sections/ModelsSection.qml llamaDetailBlock`; helpers `_weightBytes`, `_kvEstimateBytes`, `_kvDtypeBits`, `_mtpSplit`, `_percentLayersOnGPU/CPU` |
...
- Optional Tier-4: bounded `models.ini` reader for the **stopped** state so the
  available-models list shows preset intent without a running API.
